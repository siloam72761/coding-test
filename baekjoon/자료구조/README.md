...
## 📋 문제 설명  
- 프린터는 현재 Queue에 있는 문서의 중요도를 확인해서,
- 현재 문서보다 중요도가 높은 문서가 뒤에 있으면 대기열의 맨 뒤로 보낸다.
- 그렇지 않으면 해당 문서를 인쇄한다.
- 특정 문서가 몇 번째로 인쇄되는지를 출력하는 문제다.
...
## ✅ 핵심 포인트 정리  
- `(문서의 인덱스, 중요도)` 형태로 큐에 저장해 추적할 수 있어야 한다.  
- 매 루프마다 가장 큰 중요도를 가진 문서가 큐 안에 있는지를 확인해야 한다.  
- 현재 문서보다 높은 중요도의 문서가 있다면 맨 뒤로 보낸다.  
- 타겟 문서가 인쇄되는 시점을 카운트하여 출력한다.

---

## 🗒️ 배운 점 & 느낀 점  
- 처음에는 인덱스를 직접 따라가면서 큐로 구현했지만 상당히 비효율적이고, 오류가 날 가능성이 높다.
- enumerate를 이용해서 (인덱스, 우선순위)를 만들고, any를 이용하면 쉽게 구현이 가능하다.
- enumerate, any, comprehension 등의 개념을 새로 알게 된 계기가 되었다.  

---

## ⏱️ 시간복잡도 분석  
- 문서가 N개이고, 각 문서를 최대 N번 비교할 수 있으므로 최악의 경우 **O(N²)**  
- 우선순위를 비교할 때마다 전체 큐를 순회해야 하기 때문  
- 단, 평균적으로는 O(N)에 가까운 시간에 동작한다.
...
## 📋 문제 설명  
- 회사에 사람들의 출입 기록이 주어진다.  
- 각 기록은 이름과 상태(enter 또는 leave)로 이루어져 있다.  
- 현재 회사에 있는 사람들의 이름을 **사전 순의 역순**으로 출력하는 문제이다.
...
## ✅ 핵심 포인트 정리  
- 출입 기록을 순서대로 처리하면서  
- enter면 집합에 추가, leave면 집합에서 제거  
- 마지막에 집합에 남아있는 사람들을 역순으로 정렬해 출력  

---

## 🗒️ 배운 점 & 느낀 점  
- 집합(set)의 빠른 추가/삭제 기능을 활용해 문제 해결  
- 정렬 조건을 명확히 이해하고 구현하는 것이 중요  

---

## ⏱️ 시간복잡도 분석  
- 입력된 모든 출입 기록에 대해 한 번씩 추가/삭제 → O(N)  
- 마지막 출력 시 정렬 → O(M log M) (M은 집합에 남은 사람 수)  
