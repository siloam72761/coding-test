...
## 📋 문제 설명  
- N개의 좌표가 주어진다.  
- 이 좌표들을 **오름차순 정렬했을 때의 인덱스**를 각 좌표의 압축값으로 정의한다.  
- 중복 좌표는 하나로 취급하며, 원래 좌표 순서에 맞게 압축 결과를 출력한다.
...
## ✅ 핵심 포인트 정리  
- `set()`을 이용해 중복 제거 후 정렬  
- `dict`를 사용해 각 숫자에 대해 압축된 인덱스를 빠르게 맵핑  
- 입력 순서를 유지한 채 압축값을 출력  

---

## 🗒️ 배운 점 & 느낀 점  
- 중복 제거와 정렬, 맵핑을 적절히 결합하는 것이 중요하다  
- 딕셔너리를 활용한 맵핑이 압축 로직을 간결하게 만들어준다  
- `enumerate()`와 딕셔너리 응용이 문제 해결에 효과적이다
