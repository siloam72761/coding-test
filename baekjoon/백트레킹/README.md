...
## 📋 문제 설명  
- 1번 컴퓨터가 바이러스에 걸림  
- 네트워크 상에서 연결된 컴퓨터도 감염됨  
- 총 몇 대의 컴퓨터가 감염되는지를 출력  
...
## ✅ 핵심 포인트 정리  
- 1번 컴퓨터에서 DFS/BFS 시작  
- 연결된 컴퓨터들을 탐색하면서 방문 표시  
- 감염된 컴퓨터 수를 세되, 시작점(1번)은 제외  

---

## 🗒️ 배운 점 & 느낀 점  
- 8월 19일에 못 풀었던 문제를 비슷한 유형으로 다시 풀어봤으나, 코드는 전체적으로 완성했으나 로직에 몇 가지 문제가 있어서 제대로 실행이 되지 않았다. 완벽하게 풀 때 까지 계속 풀어볼 예정.
- 변수를 제대로 초기화 하지 않으면, 지역 변수로 판단하면서 코드가 제대로 굴러가지 않는다.


---

## ⏱️ 시간복잡도 분석  
- 정점(V), 간선(E)을 모두 탐색 → O(V + E)  
