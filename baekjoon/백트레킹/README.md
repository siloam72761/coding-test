...
## 📌 문제 설명
자연수 N과 M이 주어졌을 때, 아래 조건을 만족하는 길이가 M인 수열을 모두 구하는 문제입니다.

1. 1부터 N까지 자연수 중에서 M개를 고른다.  
2. 같은 수를 여러 번 골라도 된다.  
3. 고른 수열은 **비내림차순**이어야 한다.  
   - 즉, A1 ≤ A2 ≤ ... ≤ AM 조건을 만족해야 한다.  
...
## ✨ 핵심 포인트 정리
- `for` 반복문의 시작 지점을 `start`로 제한하면 이전에 선택한 수보다 작은 수를 선택하지 않는다.  
- `dfs(i, depth+1)`로 호출하면 같은 수를 다시 선택할 수 있어 중복 선택이 가능하다.  
- `visited` 배열이 필요하지 않다. (오름차순 제약이 자동으로 중복을 막아줌)  

---
## ✨ 배운점 & 느낀점
- 문제 유형이 비슷해서 하나씩만 응용해서 풀이하는 느낌이 강하다.
- 이와 같이 문제의 유형에서 하나씩 응용하는 것을 보아, 큰 틀의 코드만 머리속에 있으면 쉽게 구현가능할 것 같다는 깨달음을 얻었다.
